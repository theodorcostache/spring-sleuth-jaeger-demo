...
java -jar build/libs/open-tracing-demo-0.0.1-SNAPSHOT.jar > app.log & 
PIDS+=($!)
...
for i in `seq 1 3`;
do
    PORT=808$i NAME=PROXY$i SERVICE_URL=http://localhost:808$(($i-1)) java -jar build/libs/open-tracing-demo-0.0.1-SNAPSHOT.jar > proxy$i.log & 
    PIDS+=($!)
done    
